...
//                    StringUtils.showToast(mContext, "");
    ...
    public int getContentLayoutId() {
        return com.aaron.interview.R.layout.activity_edittext_multiple;
    }

    @Override
    public void findView() {
        editText = (EditText) findViewById(com.aaron.interview.R.id.edittext_multiple_et);
        tv_num = (TextView) findViewById(com.aaron.interview.R.id.edittext_multiple_tv_num);
        tv_changenum = (TextView) findViewById(com.aaron.interview.R.id.edittext_multiple_tv_changenum);
        btn_save = (Button) findViewById(com.aaron.interview.R.id.head_right_button);
        container = (RelativeLayout) findViewById(com.aaron.interview.R.id.container);
        setListeners();
    }
    ...
    public void init() {
        super.init();
        setRightButton("确定");
        actionbarView.setTitleColor(getColorFromResuource(com.aaron.interview.R.color.common_title));
        actionbarView.setTitleSize(MathUtils.dip2px(mContext, 18));
        container.setBackgroundResource(com.aaron.interview.R.drawable.common_bg);
        // 4.4版本以上  并且不是主页  显示statusView
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            parent.setBackgroundResource(com.aaron.interview.R.mipmap.common_bottom_bg);
            actionbarView.setBackgroundColor(getColorFromResuource(com.aaron.interview.R.color.blank));
